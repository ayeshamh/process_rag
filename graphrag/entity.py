...
class Entity:
    ...
    def __init__(self, label: str, attributes: list[Attribute], description: str = ""):
        ...
        """
        # Apply basic cleaning (remove special characters except underscores and German umlauts)
        self.label = re.sub(r"([^a-zA-Z0-9_äöüßÄÖÜ])", "", label)
    ...
    def merge(self, entity2: "Entity") -> "Entity":
        """Merge entity2 into self, concatenating descriptions when entities have same label and name.
        ...
        # Concatenate descriptions if both entities have descriptions
        if entity2.description and self.description:
            # Avoid duplicate concatenation
            if entity2.description not in self.description:
                self.description = self.description + "\n" + entity2.description
                logger.debug(f"Concatenated descriptions for entity {self.label}")
        elif entity2.description and not self.description:
            self.description = entity2.description
            logger.debug(f"Set description for entity {self.label} from entity2")
