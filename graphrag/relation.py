...
import json
import logging
from .attribute import Attribute, EDGE_LABEL_REGEX, NODE_LABEL_REGEX, EDGE_REGEX
...
class Relation:
    ...
    def from_json(txt: Union[dict, str]) -> "Relation":
        ...
        return Relation(
            txt["label"],
            _RelationEntity(txt["source"]) if isinstance(txt["source"], str) else _RelationEntity.from_json(txt["source"]),
            _RelationEntity(txt["target"]) if isinstance(txt["target"], str) else _RelationEntity.from_json(txt["target"]),
            (
